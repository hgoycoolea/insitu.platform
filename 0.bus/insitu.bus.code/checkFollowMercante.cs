...
{
    public class checkFollowMercante : IHttpHandler
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                string id_mercante = string.Empty;
                ...
                foreach (KeyValuePair<string, string> kvp in Collection)
                {
                    ...
                    if (kvp.Key == "__idc")
                    {
                        id_cliente = kvp.Value;
                    }
                    /// value for the parse id
                    if (kvp.Key == "__idm")
                    {
                        id_mercante = kvp.Value;
                    }
                }
                /// json
                string json = JsonConvert.SerializeObject(MerchantLogics.CheckFollower(int.Parse(id_mercante),int.Parse(id_cliente)));
                /// context response
