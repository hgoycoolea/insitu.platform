...
{
    public class changePromocion : IHttpHandler
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                foreach (KeyValuePair<string, string> kvp in Collection)
                {
                    ...
                    /// value for the parse estado promocion
                    if (kvp.Key == "__e")
                    {
                        estado = kvp.Value;
                    }
                ...
                /// json
                string json = JsonConvert.SerializeObject(ClientLogics.EstadoPromocion(int.Parse(id_compra),int.Parse(estado)));
                /// context response
